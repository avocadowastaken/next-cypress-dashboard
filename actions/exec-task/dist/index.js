...
async function findDeploymentURL() {
  ...
  for await (const { data: deployments } of octokit.paginate.iterator(
    ...
    (0, import_core.info)(
      deployments.length === 0 ? "No deployment found" : deployments.length === 1 ? "One deployment found" : `${deployments.length} deployments found`
    );
    for (const { id } of deployments) {
      ...
      const { data: statuses } = await octokit.request(
        ...
        { ...import_github.context.repo, deployment_id: id }
      );
      (0, import_core.info)(
        statuses.length === 0 ? "No deployment status found" : statuses.length === 1 ? "One deployment status found" : `${statuses.length} deployment statuses found`
      );
...
async function main() {
  const deploymentURL = await (0, import_core.group)(
    `Getting deployment for the environment: ${environment}`,
    findDeploymentURL
  );
  if (!deploymentURL) {
    return (0, import_core.warning)(
